...
ListContainer.prototype.hide = function (timeout$$1) {
   var self = this,
        parentContainer = self.parentListCon,
        onHideFn = self.onhide,
        timer$$1 = timeout$$1 === undefined ? DEFAULT_TIMEOUT : timeout$$1;

    if (parentContainer) {
        parentContainer.hide(timer$$1);
    }
    this.visible = false;
...
DropDownMenu.prototype.namespace = function (namespace$$1) {
...
DropDownMenu.prototype.add = function (listItems, refTo) {
    ...
    selection$$1 = parentContainer.selectAll('div').filter(filterChildNodes).data(containerData);

    selection$$1.enter().append('div').each(initContainer);
    ...
    (function recursiveParser (items, fCon, parentListItem) {
        ...
        selection$$1 = container.selectAll('div').data(listItems);

        selection$$1.exit().style('display', 'none');

        selectionEnter = selection$$1.enter().append('div').merge(selection$$1);
...
var touches$1 = function(node, touches) {
  ...
  for (var i = 0, n = touches ? touches.length : 0, points = new Array(n); i < n; ++i) {
    points[i] = point$6(node, touches[i]);
  }
...
var require$$0$1 = ( index$4 && index$4['default'] ) || index$4;

var d3Tooltip = createCommonjsModule(function (module, exports) {
(function (global, factory) {
	'object' === 'object' && 'object' !== 'undefined' ? factory(exports, require$$0$1, SmartlabelManager) :
	typeof undefined === 'function' && undefined.amd ? undefined(['exports', 'd3-selection', 'fusioncharts-smartlabel'], factory) :
	(factory((global.d3 = global.d3 || {}),global.d3,global.d3));
}(commonjsGlobal, (function (exports,d3Selection,SmartLabelManager) { 'use strict';

SmartLabelManager = 'default' in SmartLabelManager ? SmartLabelManager['default'] : SmartLabelManager;
...
Mounter.SEPARATOR = {
	...
	_def: function (smartlabel, lines, token) {
		...
		if (!isInvalid) {
			for (i1 = 0, l1 = lines.length; i1 < l1; i1++) {
				lineSize.push(smartlabel.getOriSize(lines[i1][0]).width);
			}
		} else {
			lineSize.push(0);
		}
...
__proto.mount = function (context) {
	...
	// @todo make it parameterize. To be taken from api
	container
	  .transition()
	  .duration(!this.horizontalLimit ? 0 : (this.horizontalLimit < logicalBox.width ? 0 : 500))
...
__proto._mountAtomicComponent = function (group, container, content, offset, options) {
	...
	tspan = content.text
		...
		.data(function (d) {
			dxdyCalc = Mounter.SEPARATOR._def(
				...
				d.map(function (datum) { return lib.getArgedText(datum); }),
				group.datum().breakBy);

			if (dxdyCalc.ignore) { return []; }
	...
	content.icon
		.attr('d', function (d) {
			...
			if (dxdyCalc.ignore) { return null; } 
...
function tooltip () {
	...
	function attachTooltip (elem, evts) {
	    ...
	    elem.on('touchstart.d3-tooltip', mouseHoverHandler(0, evts, tooltipElem));
	    elem.on('touchmove.d3-tooltip', mouseHoverHandler(1, evts, tooltipElem));
	    elem.on('touchend.d3-tooltip', mouseHoverHandler(2, evts, tooltipElem));
	...
	*/
	inst.show = function (position, mounter) {
		...
		if (_constrain) {
			if (_constrain.x2) {
				if (pos[0] + _offset.x + size.width > _constrain.x2) {
					if (pos[0] - _offset.x - size.width < _constrain.x1) {
						pos[0] = _constrain.x1;
			...
			if (_constrain.y2 || heightAdjustmentNeeded) {
				_constrain.y2 = _constrain.y2 || Number.POSITIVE_INFINITY;
				if (heightAdjustmentNeeded) {
					offsetY = _offset.y * 1.2;
				} else {
					offsetY = _offset.y;
				}

				if (pos[1] + offsetY + size.height > _constrain.y2) {
	...
	*/
	...
	inst.bbox = function (x, y, width, height) {
		x = x || 0;
		y = y || 0;
		y = y || 0;
		width = width || 0;
...
Button.prototype.getLogicalSpace = function () {
    ...
    typeof symbol$$1 === 'string' && (dimensions = getTextDimensions(symbol$$1, textClass, select('svg'), smartLabel));
...
Button.prototype.draw = function (x, y, group) {
    ...
    if (typeof symbol$$1 === 'string') {
        ...
        textEl.text(symbol$$1).transition(t).
    ...
    else if (typeof symbol$$1 === 'function') {
        symbolObj = symbol$$1(x + padLeft, y + padTop, width - (padLeft + padRight),
...
InputButton.prototype.blur = function (textStr) {
    ...
    text$$1 && text$$1.style('display', 'block').text(smartText.text);
    return this;
};
...
InputButton.prototype.text = function (text$$1) {
    var inputBox = this.elements.inputBox,
        value = text$$1;

    if (text$$1) {
        inputBox && (inputBox.node().value = text$$1);
        this.blur(text$$1);
    }
    else {
...
function button (symbol$$1) {
    return new Button(symbol$$1);
}

function inputButton (symbol$$1) {
    return new InputButton(symbol$$1);
}
...
SelectButton.prototype.text = function (text$$1) {
    return this.buttonGroup.select('text').text(text$$1);
};
...
function ButtonWithContextMenu (symbol$$1, container) {
    Button.apply(this, arguments);
...
function buttonWithContextMenu (symbol$$1, container) {
    return new ButtonWithContextMenu(symbol$$1, container);
}
...
__proto.createRowsRecursively = function (container, components) {
    ...
    selection$$1 = container.selectAll('div').data(components);

    selectionEnter = selection$$1.enter().append('div');

    selectionEnter.merge(selection$$1).each(function (d) {
...
__proto.createElements = function (selection$$1, elementArr) {
    var elements = this.elements,
    ...
    for (key in elementsByType) {
        ...
        elementSelection = selection$$1.selectAll(key).data(elements);
